<script type="ts">
	...
	async function setMyTenantName() {
		...
		let ret = await api.post('tnt/set/name', { orgname: orgname }, user.sessionToken);
		if (ret.error) {
	...
	async function setMyTenantTheme() {
		...
		let ret = await api.post('tnt/set/theme', { css: orgtheme }, user.sessionToken);
		if (ret.error) {
			setFadeMessage(ret.message, 'warning');
		} else {
			//eslint-disable-next-line
	...
	async function setMyTenantTimezone() {
		...
		let ret = await api.post('tnt/set/timezone', { timezone: orgtimezone }, user.sessionToken);
		if (ret.error) {
			setFadeMessage(ret.message, 'warning');
		} else {
			//eslint-disable-next-line
