...
<script type="ts">
	...
	async function load(_page, reason = 'unknown') {
		...
		payload_extra['starter'] = $filterStorage.starter;
		if ($filterStorage.tplTag) payload_extra['tagsForFilter'] = $filterStorage.tplTag.split(';');
		if ($filterStorage.tplid.trim()) payload_extra['tplid'] = $filterStorage.tplid.trim();
	...
	async function searchNow(detail = null) {
		...
		searchTimer = setTimeout(() => {
			if (Utils.isBlank($filterStorage.tplTag)) {
				$filterStorage.tplTag = '';
			}
			if (Utils.isBlank($filterStorage.tplid)) {
				$filterStorage.tplid = '';
			}
			if (Utils.isBlank($filterStorage.starter)) {
			...
			if (Utils.isBlank($filterStorage.calendar_begin)) $filterStorage.calendar_begin = '';
			if (Utils.isBlank($filterStorage.calendar_end)) $filterStorage.calendar_end = '';
	...
	if (Utils.isBlank($filterStorage.starter)) $filterStorage.starter = user.email;
</script>
