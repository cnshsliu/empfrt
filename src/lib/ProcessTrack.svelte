<script lang="ts">
	import { Button, Row, Col, NavLink, Nav, NavItem, Icon, Input } from 'sveltestrap';
...
<div class="m-3 py-3 kfk-highlight-2 text-center position-relative">
	...
	{#if wf}
		...
		{#if $printing === false}
			<Row class="mx-5 bg-primary bg-opacity-10 justify-content-end">
				<Nav pills class="justify-content-end">
					<NavItem class="px-3">
						<NavLink
					...
					<NavItem class="px-3">
						<NavLink class="kfk-link m-0 p-0" on:click={onShowLog}>
							<AniIcon icon="journal-arrow-down" ani="aniShake" />
							&nbsp;
							{$_('todo.showlog')}
						</NavLink>
					</NavItem>
					<NavItem class="px-3">
						<NavLink
							...
							<AniIcon icon="filetype-xlsx" ani="aniShake" />
							&nbsp;
